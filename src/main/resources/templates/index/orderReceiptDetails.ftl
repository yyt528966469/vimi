...
function CreatePrintPage(json) {
    ...
    LODOP.ADD_PRINT_TEXT(hPos,60,pageWidth,rowHeight,"外卖点餐");
    hPos+=rowHeight;
    ...
    for(var i=0;i<count;i++){
        ...
        LODOP.ADD_PRINT_TEXT(hPos,30,pageWidth,rowHeight,address.substr(0,11));
        address=address.substr(11,address.length);
    }
    if(address.length>0){
        hPos+=rowHeight;
